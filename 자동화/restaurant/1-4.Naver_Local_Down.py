...
import shutil
import time
import JsonLoad as js

url="https://www.localdata.go.kr/devcenter/dataDown.do?menuNo=20001"
json_path="input_condition.jsonl"
file_id=0

tag_value=ft.partial(js.json_value_load,file_path=json_path)
...
# 식품 카테고리 ##cate_1
cate_1_dis=tag_value(id=0)
cate_1_css=tag_value(id=1)

food_1=bott(css=cate_1_css,distinct=cate_1_dis)
food_1.click()
time.sleep(3)

# 식품>음식점 카테고리 ##cate_2
cate_2_dis=tag_value(id=2)
cate_2_css=tag_value(id=3)

food_2=bott(css=cate_2_css,distinct=cate_2_dis)
food_2.click()
time.sleep(3)

# 음식점 하위 태그(관광식당, 일반음식점, 휴게음식점) 카테고리 ##cate_3
cate_3_css=tag_value(id=4)

food_3=bott(css=cate_3_css)
time.sleep(3)
